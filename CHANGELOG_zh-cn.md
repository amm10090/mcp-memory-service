...
### Added
- **混合后端漂移检测** —— 基于 `updated_at` 的自动元数据同步（Issue #202）。
  - 双向检测：发现 Cloudflare 与 SQLite-vec 间的新/改/删并触发相应同步。
  - 防止因 API 错误、网络超时、部分更新造成的隐性漂移。
- **时间戳修复脚本**：`scripts/maintenance/recover_timestamps_from_cloudflare.py`，以 Cloudflare 为真实来源恢复 v8.24.0 漂移导致的时间戳问题。
- **文档**：在 `docs/deployment/systemd-service.md` 增漂移检测章节，`README.md` 补充概览与时间戳恢复排错步骤。
## [8.24.0] - 2025-11-12

### Added
- **PyPI 发布自动化** —— 现可通过 `pip install mcp-memory-service` 获取。
  - GitHub Actions 在打 tag 时自动发布到 PyPI（使用 `PYPI_TOKEN` 机密）。
  - 安装更简：`pip install mcp-memory-service` 或 `uv pip install mcp-memory-service`。
  - 解决无 git/不熟悉源码用户的安装门槛。
  - 质量闸：仅在测试通过后发布。

### Changed
- 发行渠道：新增 PyPI 为主渠道，保留 GitHub release。
- 安装文档：更新为优先推荐 pip 安装。

### 技术细节
- 变更文件：
  - `.github/workflows/publish.yml` —— 新增自动发布流程。
  - GitHub 仓库设置：新增 `PYPI_TOKEN` 机密。
