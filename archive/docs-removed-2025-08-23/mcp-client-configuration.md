...
- **Endpoint**: `http://your-server:8001/mcp`
...
# Usage
client = MCPMemoryClient("http://your-server:8001/mcp", "YOUR_API_KEY")
...
const result = await client.storeMemory('Meeting notes', [
	'meetings',
	'important',
]);
```
...
```json
{
	"mcpServers": {
		"memory": {
			"transport": "http",
			"endpoint": "https://your-domain.com:8001/mcp",
			"headers": {
				"Authorization": "Bearer YOUR_API_KEY"
			}
		}
...
1. **Check server status**:
   ...
   ```bash
   curl -s http://your-server:8001/api/health
   ```
...
```json
{
	"mcpServers": {
		"memory": {
			"endpoints": [
				"https://memory1.yourdomain.com:8001/mcp",
				"https://memory2.yourdomain.com:8001/mcp"
			],
